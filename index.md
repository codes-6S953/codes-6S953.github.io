---
course_number: 6.S953
term: Fall 2026
course_type: Graduate · 12 units
title: Topics in Error-Correcting Codes for Computer Science
description: This topics course in error-correcting codes begins with the fundamentals, progresses to the research frontier, and explores recent developments in the field.
university: Massachusetts Institute of Technology
department: Electrical Engineering and Computer Science
mark: MIT
meets: Mon & Wed<br>11:00–12:30
room: 36-156
professor: [Yael T. Kalai](https://www.csail.mit.edu/person/yael-kalai)<br>[tauman@mit.edu](mailto:tauman@mit.edu)
ta_co_lecturer: [Rohan Goyal](https://goyal-rohan.github.io)<br>[rohan_g@mit.edu](mailto:rohan_g@mit.edu)
---

# Schedule

Classes meet Mondays and Wednesdays from 11:00–12:30. Calendar exceptions are noted below.

Use a row beginning with `MODULE` to add a section divider. Separate multiple links or notes in the final column with `<br>`.

| Course no. & date | Lecturer | Topic | Notes & references |
| --- | --- | --- | --- |
| MODULE 01 |  | Foundations |  |
| Lecture 01<br>Wed · Sep 09 | Yael Kalai | Introduction and basics | [Lecture 1 notes](files/ECC-lec-1.pdf) 
Chapter 1 [GRS](https://cse.buffalo.edu/faculty/atri/courses/coding-theory/book/) |
| Lecture 02<br>Mon · Sep 14 | Rohan Goyal | Linear codes and q-ary entropy | Chapters 2 and 3 [GRS](https://cse.buffalo.edu/faculty/atri/courses/coding-theory/book/) |
| Lecture 03<br>Wed · Sep 16 | Rohan Goyal | GV and Singleton bounds; Reed-Solomon and MDS codes | Chapters 4 and 5 [GRS](https://cse.buffalo.edu/faculty/atri/courses/coding-theory/book/) |
| Lecture 04<br>Mon · Sep 21 | Rohan Goyal | List-decoding bounds: capacity, Johnson, and generalized Singleton | Chapters 6–8 [GRS](https://cse.buffalo.edu/faculty/atri/courses/coding-theory/book/) |
| MODULE 02 |  | Interactive coding theory |  |
| Lecture 05<br>Wed · Sep 23 | Yael Kalai | The interactive and feedback models | — |
| Lecture 06<br>Mon · Sep 28 | Yael Kalai | — | — |
| Lecture 07<br>Wed · Sep 30 | Yael Kalai | — | — |
| Lecture 08<br>Mon · Oct 05 | Yael Kalai | — | — |
| MODULE 03 |  | Code constructions |  |
| Lecture 09<br>Wed · Oct 07 | Rohan Goyal | Polynomial codes | — |
| No class<br>Mon · Oct 12 | — | — | Indigenous Peoples' Day · `Institute holiday` |
| Lecture 10<br>Tue · Oct 13 | Guest | — | `Monday schedule of classes` |
| Lecture 11<br>Wed · Oct 14 | Guest | — | — |
| MODULE 04 |  | Subspace designs and LCL properties |  |
| Lecture 12<br>Mon · Oct 19 | — | — | — |
| Lecture 13<br>Wed · Oct 21 | — | — | — |
| Lecture 14<br>Mon · Oct 26 | — | — | — |
| Lecture 15<br>Wed · Oct 28 | — | — | — |
| Midterm<br>Mon · Nov 02 | — | — | `Midterm` |
| MODULE 05 |  | Algorithms |  |
| Lecture 16<br>Wed · Nov 04 | — | — | — |
| Lecture 17<br>Mon · Nov 09 | — | — | — |
| No class<br>Wed · Nov 11 | — | — | Veterans Day · `Institute holiday` |
| Lecture 18<br>Mon · Nov 16 | — | — | — |
| Lecture 19<br>Wed · Nov 18 | — | — | — |
| MODULE 06 |  | Applications |  |
| Lecture 20<br>Mon · Nov 23 | — | — | — |
| Lecture 21<br>Wed · Nov 25 | — | — | — |
| Lecture 22<br>Mon · Nov 30 | — | — | — |
| Lecture 23<br>Wed · Dec 02 | — | — | — |
| Project presentations<br>Mon · Dec 07 | — | — | `Presentations` |
| Project presentations<br>Wed · Dec 09 | — | — | `Presentations` · `Last course meeting` |

# Overview

This topics course in error-correcting codes begins with the fundamentals, progresses to the research frontier, and explores recent developments in the field. Principal areas include existence results and limitations for error-correcting codes, constructions and algorithms, interactive coding, subspace designs, and applications in computer science.

## Prerequisites

6.1200, 6.1210, and 18.06.

# Syllabus

The course is organized into the following modules. Lecture-level topics, notes, and references will be added to the schedule as the course develops. The midterm is scheduled for November 2, and the final two class meetings are reserved for project presentations.

## 1. Foundations: Possibilities and limitations; Chapters 1–8 of [GRS](https://cse.buffalo.edu/faculty/atri/courses/coding-theory/book/web-coding-book.pdf)

## 2. Interactive coding theory: Limitations of and improvements to error correction through interaction and feedback

## 3. Code constructions: Algebraic and combinatorial techniques; Chapters 9–11 of [GRS](https://cse.buffalo.edu/faculty/atri/courses/coding-theory/book/web-coding-book.pdf) and more recent developments

## 4. Subspace designs and LCL properties: Constructions of subspace-design codes, combinatorial techniques for list decoding, and equivalences among LCL properties

## 5. Algorithms: List-decoding algorithms for Reed-Solomon and expander-based codes

## 6. Applications: A range of applications across theoretical computer science

# Logistics

## Level and units

Graduate · 12 units

## Prerequisites

6.1200, 6.1210, and 18.06

## Requirement

AAGS in Theoretical Computer Science

# References

## GRS

Venkatesan Guruswami, Atri Rudra, and Madhu Sudan, [*Essential Coding Theory*](https://cse.buffalo.edu/faculty/atri/courses/coding-theory/book/), draft book.

## Sud23

Madhu Sudan, [*Essential Coding Theory*](https://docs.google.com/document/d/1UnpFN34N2su7neKZJv8geeLUy_tzB5J4pBlAVJvBYGQ/edit?tab=t.0#heading=h.6c0r0a61enx8), course materials.

## Har24

Prahladh Harsha, [*CSS.318.1: Coding Theory*](https://tifr-css-318-1-coding-theory-2024.bitbucket.io/), TIFR, 2024.

## Woo25

Mary Wootters, [*CS250/EE387: Error-Correcting Codes*](https://web.stanford.edu/class/cs250/), Stanford University, Winter 2025. [Lecture videos](https://www.youtube.com/playlist?list=PLkvhuSoxwjI_UudECvFYArvG0cLbFlzSr).

## Gur22

Venkatesan Guruswami, [*CS 294-226: Advances in Error-Correcting Codes*](https://people.eecs.berkeley.edu/~venkatg/teaching/ECC-fall22/), UC Berkeley, Fall 2022.

## Kop22

Swastik Kopparty, [*Topics in Error Correcting Codes*](https://www.math.utoronto.ca/swastik/courses/codes-F22/), University of Toronto, Fall 2022.

# Policies

## Evaluation

The grade will be based on the following three components:

- **Course project — 50%.** See the project details and timeline below.
- **In-class midterm — 20%.**
- **Problem sets — 30%.** See the problem-set details below.

## Project

The project is completed in groups of two or three and may take one of two forms: research or reading.

Candidate projects for both forms will be added soon. You may also propose a topic that aligns with the course, subject to approval by the professor or TA. You may schedule a meeting with Rohan to discuss possible projects and ideas.

Project timeline:

- **October 2 at 6:00 PM — Topic and group selection (5%).**
- **October 16 at 6:00 PM — One-page project proposal and plan (10%).**
- **November 13 at 6:00 PM — One-page progress report (10%).**
- **December 7 and 9 — Project presentations (15%).**
- **December 11 — Eight- to ten-page project report or paper (10%).**

## Problem sets

There will be four problem sets, and the final grade will be based on your three highest scores. Each problem set will contain four to six problems, of which you are expected to solve at least three. You will then schedule a 15-minute meeting with the TA to present your solutions and answer questions about the three problems you chose.

## Collaboration

Students may collaborate with classmates and use AI models to discuss course material. We strongly recommend limiting AI use to supporting understanding rather than solving problems. Students are responsible for all content in their projects, presentations, and submitted work.
